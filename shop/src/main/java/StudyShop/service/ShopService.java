...
public class ShopService
{
    ...
    public void submitOrder(HttpSession ses, String city)
    {
        if(getCart(ses).GetProductsCount() == 0)
            return;
        String productsStr = "";
        Cart wishList = getCart(ses);
        ProductEncounter cnts = wishList.GetProductCounts();
        for (ProductEncounter.ProdPair pair : cnts.counts)
        {
            if(!productsStr.isEmpty()) productsStr+=", ";
            productsStr += pair.productId.toString();
            productsStr += " x";
            productsStr += pair.count.toString();
        }
        Order order = new Order();
        order.setCity(city);
        order.setDate(new Date());
        order.setItems(productsStr);
        wishList.Clean();
